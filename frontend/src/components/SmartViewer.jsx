...
export default function SmartViewer({
  ...
  const [viewMode, setViewMode] = useState(VIEW_MODES.FULLPAGE)
  ...
  const fullpageShot = screenshot?.type === 'fullpage' ? screenshot : null
  const fullpageId = screenshot?.parent_id || screenshot?.id

  const selectedSmart = smartVersions.find(s => s.id === selectedSmartId) ||
    smartVersions.find(s => s.strategy === selectedStrategy && !s.is_manual_region) ||
    smartVersions[0]
  ...
  useEffect(() => {
    if (selectedSmart) {
      ...
      const defaultRegion = allRegions.find(r => r.strategy === selectedStrategy) || allRegions[0]
      if (defaultRegion) {
        setCurrentRegion({
  ...
  const handleReAnalyze = async () => {
    setReanalyzing(true)
    try {
      const res = await reAnalyzeScreenshot(fullpageId, selectedStrategy)
      await loadRegions()
  ...
  const getDisplayImage = () => {
    if (viewMode === VIEW_MODES.SMART && selectedSmart) {
      return {
        ...
        label: `智能裁剪 - ${STRATEGY_LABELS[selectedSmart.strategy] || selectedSmart.strategy}`
      }
    }
    return {
      ...
      <div className="bg-gray-800 px-6 py-3 flex flex-wrap gap-3 items-center">
        <div className="flex gap-1 bg-gray-700 rounded-lg p-1">
          ...
          <button
            ...
            disabled={smartVersions.length === 0}
            className={`px-3 py-1.5 text-sm rounded-md transition-colors ${
              viewMode === VIEW_MODES.SMART
                ? 'bg-blue-600 text-white'
                : 'text-gray-300 hover:text-white hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed'
            }`}
          >
            智能裁剪
              ...
              >
                {smartVersions.map(s => (
                  <option key={s.id} value={s.id}>
                    ...
                    {s.strategy === selectedSmart?.strategy ? ' ✓' : ''}
                  </option>
                ))}
              </select>
            </div>
        ...
        <div className="text-sm text-gray-400">
          ...
          <span className="ml-3 text-gray-500">
            {((displayImg.width * displayImg.height) / 1000000).toFixed(2)} MP
          </span>
        </div>
      </div>
      ...
      {viewMode === VIEW_MODES.EDIT ? (
                  ...
                  >
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-xs font-medium px-2 py-0.5 rounded" style={{
                        backgroundColor: r.strategy === 'dom' ? '#3b82f633' :
                          r.strategy === 'visual' ? '#10b98133' :
                          r.strategy === 'hybrid' ? '#8b5cf633' : '#f59e0b33',
                        color: r.strategy === 'dom' ? '#60a5fa' :
                          r.strategy === 'visual' ? '#34d399' :
                          r.strategy === 'hybrid' ? '#a78bfa' : '#fbbf24'
                      }}>
        ...
        <div className="flex-1 overflow-auto flex items-center justify-center p-6 bg-gray-950">
          <div className="relative">
            <div className="text-xs text-gray-500 mb-2 text-center">{displayImg.label}</div>
            <img
              src={displayImg.src}
            ...
            {viewMode === VIEW_MODES.SMART && selectedSmart && (
              <div className="mt-2 text-xs text-gray-400 text-center">
                裁剪区域: ({selectedSmart.region_x}, {selectedSmart.region_y}) - {selectedSmart.region_width}×{selectedSmart.region_height}
